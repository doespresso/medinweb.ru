...
class RussianPost implements IService
{
    ...
    public function getRequestData()
    {
        ...
        if ($origination['country'] != 'RU') {
            $this->_internalError(__('russian_post_country_error'));
        }

        if ($origination['state'] != 'MOW') {
            $this->_internalError(__('russian_post_city_error'));
        }
        ...
        $russian_post_request_settings = array(
            ...
            'shipping_type' => array(
                'ground' => 1,
                'air' => 2,
                //          'combin' => 3, // do not work. return '-' for some reasons.
                //          'express' => 4 // do not work. return '-' for some reasons.
            )
        );
