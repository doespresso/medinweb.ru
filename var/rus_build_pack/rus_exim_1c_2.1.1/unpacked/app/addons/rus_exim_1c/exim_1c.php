...
if ($type == 'catalog') {
    if ($mode == 'checkauth') {
        ...
        if ($fileinfo['filename'] == 'import') {
            ...
            fn_exim_1c_import_product_image_from_ftp($xml);
        }
        fn_echo("success\n");
    } elseif ($mode == 'get_dir') {
        fn_echo('var/exim/1C_' . date('dmY') . '/'); 
    } elseif ($mode == 'create_dir') {
        if (!is_dir(DIR_C)) {
            fn_mkdir(DIR_C);
            fn_echo ("dir create\n");
            fn_echo (DIR_C);
    ...
    } elseif ($mode == 'del_imagesxml') {
        fn_rm('images.xml', false, DIR_C);
        fn_echo ('remove image.xml');
    }
