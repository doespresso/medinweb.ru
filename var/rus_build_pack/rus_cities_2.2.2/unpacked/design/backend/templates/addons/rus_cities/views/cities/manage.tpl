...
<tr class="cm-row-status-{$city.status|lower}">
    ...
    <td class="nowrap">
        {capture name="tools_list"}
            <li>{btn type="list" class="cm-confirm" text=__("delete") href="cities.delete?city_id=`$city.city_id`&state_code=`$search.state_code`&country_code=`$search.country_code`"}</li>
        {/capture}
...
<div class="sidebar-field">
    ...
    <label id="elm_states_lbl" {if empty($states.$_country)}class="hidden"{/if}>{__("state")}:</label>
		...
		<div class="hidden"><input type="text" id="elm_states_d" name="state_code" size="32" maxlength="64" value="{$_state}" disabled="disabled" readonly="readonly" class="cm-state cm-location-states input-large hidden cm-skip-avail-switch" style="border:0px; background-color: transparent;"/></div>
        <span id="elm_states_empty" {if !empty($states.$_country)}class="hidden"{/if}>{__("empty_state")}</br><a href="admin.php?dispatch=states.manage&country_code={$_country}">{__("new_city_state")}</a></span>
</div>
