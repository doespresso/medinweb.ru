...
function fn_exim_yml_get_product_features($product_id, $lang_code = CART_LANGUAGE)
{
    static $features;

    if (!isset($features[$lang_code])) {
        list($features) = fn_get_product_features(array('plain' => true), 0, $lang_code);
    }
    ...
    if (!empty($product_features)) {
        foreach ($product_features as $f) {
            $display_on_catalog = $features[$f['feature_id']]['display_on_catalog'];
            $display_on_product = $features[$f['feature_id']]['display_on_product'];
            if ($display_on_catalog == "Y" || $display_on_product == "Y") {
                if ($f['feature_type'] == "C") {
                    $result[] = array(
                        'description' => fn_exim_yml_get_product_info($f['description']),
                        'value' => ($f['value'] == "Y") ? __("yes") : __ ("no")
                    );
                } elseif ($f['feature_type'] == "S" && !empty($f['variant'])) {
                    $result[] = array(
                        'description' => fn_exim_yml_get_product_info($f['description']),
                        'value' => fn_exim_yml_get_product_info($f['variant'])
                    );
                } elseif ($f['feature_type'] == "T" && !empty($f['value'])) {
                    $result[] = array(
                        'description' => fn_exim_yml_get_product_info($f['description']),
                        'value' => fn_exim_yml_get_product_info($f['value'])
                    );
                    ...
                    if (!empty($f['variants'])) {
                        ...
                        $result[] = array(
                            'description' => fn_exim_yml_get_product_info($f['description']),
                            'value' => fn_exim_yml_get_product_info($_value)
                        );
                    }
                    ...
                    $result[] = array(
                        'description' => fn_exim_yml_get_product_info($f['description']),
                        'value' => fn_exim_yml_get_product_info($f['value_int'])
                    );
                }
...
function fn_exim_yml_get_image_url($product_id, $object_type, $pair_type, $get_icon, $get_detailed, $lang_code)
{
...
function fn_exim_yml_get_product_url($product_id, $lang_code = '')
{
    return fn_url("products.view?product_id=$product_id", 'C', 'http', $lang_code);
}

function fn_exim_yml_export_price($price, $decimals_separator)
{
    return fn_format_rate_value($price,'F',2,$decimals_separator,'','');
}

function fn_exim_yml_get_product_info($data)
{
