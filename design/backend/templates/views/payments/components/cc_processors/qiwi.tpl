...
{assign var="s_url" value="payment_notification.process?payment=qiwi"|fn_url:'C':'http'}
{assign var="f_url" value="payment_notification.checkout_redirect?payment=qiwi"|fn_url:'C':'http'}
{assign var="q_url" value="http"|fn_payment_url:"qiwi.php?parameter=update"}
<p>{__("qiwi_url_notice", ["[success_url]" => $s_url, "[fail_url]" => $f_url, "[qiwi_url]" => $q_url])}</p>
<p>{__("qiwi_login_notice")}</p>
<p>{__("qiwi_alarm_notify")}</p>
<hr>

<div class="control-group">
    <label class="control-label" for="qiwi_login">{__("login")}:</label>
    <div class="controls">
...
<div class="control-group">
    ...
    <div class="controls">
        <select name="payment_data[processor_params][lifetime]" id="qiwi_lifetime">
            <option value="60" {if $processor_params.lifetime == "60"}selected="selected"{/if}>1 {__("rus_build_pack.hour")}</option>
            <option value="720" {if $processor_params.lifetime == "720"}selected="selected"{/if}>12 {__("rus_build_pack.how_hours")}</option>
            <option value="1440" {if $processor_params.lifetime == "1440"}selected="selected"{/if}>1 {__("rus_build_pack.day")}</option>
            <option value="10080" {if $processor_params.lifetime == "10080"}selected="selected"{/if}>7 {__("rus_build_pack.how_days")}</option>
            <option value="20160" {if $processor_params.lifetime == "20160"}selected="selected"{/if}>14 {__("rus_build_pack.how_days")}</option>
            <option value="43200" {if $processor_params.lifetime == "43200"}selected="selected"{/if}>30 {__("rus_build_pack.how_days")}</option>
        </select>
    </div>
</div>
